...
public class UntisUtils
{
	 ...
	 *
	 * @param method
	 *            method you want to process
	 * @param optionalParams
	 *            params you want to process
			...
			Map<String, ?> optionalParams)
	{
		...
		return "{\"id\":\"ID\",\"method\":\"" + method
				+ "\",\"jsonrpc\":\"2.0\",\"params\":" + paramsJSONObject + "}";
	}

	/**
	 * Returns a well formatted {@code java.util.HashMap} with date params in
	 * it, which can be used to send date information to the untis server
	 *
	 * @param start
	 ...
	 * @since 1.0
	 */
	public static HashMap<String, String> localDateToParams(LocalDate start,
			LocalDate end)
	{
		if (end.isBefore(start))
		{
			throw new DateTimeException(
					"The end date must end after or on the same day as the start date");
		}
	 ...
	 * @param list2
	 *            list 2 to check
	 * @return {@code true} if the two lists contains a same item, {@code false}
	 *         if not
	 * @since 1.0
	 */
	 ...
	 * @param set2
	 *            set 2 to check
	 * @return {@code true} if the two sets contains a same item, {@code false}
	 *         if not
	 * @since 1.0
	 */
	 ...
	 *
	 * <p>
	 * This enum contains all element types that can be used while POST a method
	 * to the untis server
	 * </p>
	 *
	 * @since 1.0
	...
	public enum ElementType
	{
		 ...
		 *
		 * @param elementType
		 *            element type int from which you want to get the
		 *            {@link ElementType}
		...
		public static ElementType of(int elementType)
		{
			...
			{
				throw new EnumConstantNotPresentException(ElementType.class,
						"Invalid value for ElementType: " + elementType);
			}
	 ...
	 *
	 * <p>
	 * This enum contains all methods that can be POSTed to the untis server and
	 * support units4j
	 * </p>
	 *
	 * @since 1.0
	...
	public enum Method
	{
		LOGIN("authenticate"), LOGOUT("logout"),

		GETCLASSREGCATEGORIES(
				"getClassregCategories"), GETCLASSREGCATEGORYGROUPS(
						"getClassregCategoryGroups"), GETCLASSREGEVENTS(
								"getClassregEvents"), GETCURRENTSCHOOLYEAR(
										"getCurrentSchoolyear"), GETDEPARTMENTS(
												"getDepartments"), GETEXAMS(
														"getExams"), GETEXAMTYPES(
																"getExamTypes"), GETHOLIDAYS(
																		"getHolidays"), GETCLASSES(
																				"getKlassen"), GETLATESTIMPORTTIME(
																						"getLatestImportTime"), GETROOMS(
																								"getRooms"), GETSCHOOLYEARS(
																										"getSchoolyears"), GETSTATUSDATA(
																												"getStatusData"), GETSUBJECTS(
																														"getSubjects"), GETTEACHERS(
																																"getTeachers"), GETTIMEGRIDUNTIS(
																																		"getTimegridUnits"), GETTIMETABLE(
																																				"getTimetable"), GETTIMETABLEWITHABSENCE(
	...
	{
		CANCELLED("cancelled"), IRREGULAR("irregular"), REGULAR("regular");
