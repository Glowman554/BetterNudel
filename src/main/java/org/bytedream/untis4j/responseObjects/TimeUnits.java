...
public class TimeUnits extends ResponseList<TimeUnits.TimeUnitObject>
{
	 ...
	 *
	 * @param timeUnits
	 *            time units that should be sorted
	...
	/**
	 * Sorts the given time units by all start times and returns the sorted time
	 * units
	 *
	 * @param timeUnits
	 *            time units that should be sorted
	...
	/**
	 * Sorts the given time units by all end times and returns the sorted time
	 * units
	 *
	 * @param timeUnits
	 *            time units that should be sorted
	 ...
	 *
	 * @param name
	 *            name of the time unit you want to find
	 * @return the time unit
	 * @since 1.0
	 */
	public TimeUnitObject findByName(String name)
	{
		return this.stream().filter(timeUnitObject -> timeUnitObject.getName()
				.equalsIgnoreCase(name.trim())).findAny().orElse(null);
	}
	 ...
	 *
	 * @param startTime
	 *            start time of the time unit you want to find
	 * @return the time unit
	 * @since 1.0
	 */
	public TimeUnitObject findByStartTime(LocalTime startTime)
	{
		return this.stream().filter(timeUnitObject -> timeUnitObject
				.getStartTime().equals(startTime)).findAny().orElse(null);
	}
	 ...
	 *
	 * @param endTime
	 *            end time of the time unit you want to find
	 * @return the time unit
	 * @since 1.0
	 */
	public TimeUnitObject findByEndTime(LocalTime endTime)
	{
		return this.stream().filter(
				timeUnitObject -> timeUnitObject.getEndTime().equals(endTime))
				.findAny().orElse(null);
	}
	 ...
	 *
	 * @param name
	 *            name of the time units you want to search
	 * @return {@link TimeUnits} with time units that have the {@code name} or a
	 *         part of it in their name
	 * @since 1.0
	 */
	public TimeUnits searchByName(String name)
	{
		...
		this.stream()
				.filter(timeUnitObject -> timeUnitObject.getName().toLowerCase()
						.contains(name.trim().toLowerCase()))
				.forEach(timeUnits::add);

		return timeUnits;
	}

	/**
	 * Finds time units that have the {@code startTime} or a part of it in their
	 * start time
	 *
	 * @param startTime
	 *            start time of the time units you want to search
	 * @return {@link TimeUnits} with time units that have the {@code startTime}
	 *         or a part of it in their start time
	 * @since 1.0
	 */
	public TimeUnits searchByStartTime(LocalTime startTime)
	{
		...
		this.stream()
				.filter(timeUnitObject -> timeUnitObject.getStartTime()
	...
	/**
	 * Finds time units that have the {@code endTime} or a part of it in their
	 * end time
	 *
	 * @param endTime
	 *            end time of the time units you want to search
	 * @return {@link TimeUnits} with time units that have the {@code endTime}
	 *         or a part of it in their end time
	 * @since 1.0
	 */
	public TimeUnits searchByEndTime(LocalTime endTime)
	{
		...
		this.stream()
				.filter(timeUnitObject -> timeUnitObject.getEndTime()
						.format(DateTimeFormatter.ofPattern("HHmmss"))
						.contains(endTime
								.format(DateTimeFormatter.ofPattern("HHmmss"))))
				.forEach(timeUnits::add);
	...
	public ArrayList<LocalTime> getStartTimes()
	{
		...
		this.stream().map(TimeUnitObject::getStartTime)
				.forEach(startTimes::add);
	...
	public static class TimeUnitObject extends ResponseObject
	{
		 ...
		 * @param startTime
		 *            start time of the time unit
		 * @param endTime
		 *            end time of the time unit
		 * @since 1.0
		 */
		public TimeUnitObject(String name, LocalTime startTime,
				LocalTime endTime)
		{
			this.name = name;
