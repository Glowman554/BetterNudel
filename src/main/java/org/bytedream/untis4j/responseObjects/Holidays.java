...
public class Holidays extends NILResponseList<Holidays.HolidaysObject>
{
	...
	/**
	 * Sorts the given holidays by all start dates and returns the sorted
	 * holidays
	 *
	 * @param holidays
	 *            holidays that should be sorted
	 ...
	 *
	 * @param startDate
	 *            start date of the holidays you want to find
	 * @return the holidays
	 * @since 1.0
	 */
	public HolidaysObject findByStartDate(LocalDate startDate)
	{
		return this.stream().filter(holidaysObject -> holidaysObject
				.getStartDate().isEqual(startDate)).findAny().orElse(null);
	}
	 ...
	 *
	 * @param endDate
	 *            end date of the holidays you want to find
	 * @return the holidays
	 * @since 1.0
	 */
	...
	{
		return this.stream().filter(holidaysObject -> holidaysObject
				.getStartDate().isEqual(endDate)).findAny().orElse(null);
	}

	/**
	 * Finds holidays that have the {@code startDate} or a part of it in their
	 * start date
	 *
	 * @param startDate
	 *            start date of the holidays you want to search
	 * @return {@link Holidays} with holidays that have the {@code startDate} or
	 *         a part of it in their start date
	 * @since 1.0
	 */
	public Holidays searchByStartDate(LocalDate startDate)
	{
		...
		this.stream()
				.filter(holidaysObject -> holidaysObject.getStartDate()
	...
	/**
	 * Finds holidays that have the {@code endDate} or a part of it in their end
	 * date
	 *
	 * @param endDate
	 *            end date of the holidays you want to search
	 * @return {@link Holidays} with holidays that have the {@code endDate} or a
	 *         part of it in their end date
	 * @since 1.0
	 */
	...
	public ArrayList<LocalDate> getStartDates()
	{
		...
		this.stream().map(HolidaysObject::getStartDate)
	...
	public static class HolidaysObject extends NILResponseObject
	{
		 ...
		 * @since 1.0
		 */
		public HolidaysObject(String name, int id, String longName,
