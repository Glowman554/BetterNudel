...
public class TimegridUnits
		extends ResponseList<TimegridUnits.TimegridUnitObject>
{

	/**
	 * Sorts the given timegrid units by all days and returns the sorted
	 * timegrid units
	 *
	 * @param timegridUnits
	 *            timegrid units that should be sorted
	...
	/**
	 * Sorts the given timegrid units by all time units and returns the sorted
	 * timegrid units
	 *
	 * @param timegridUnits
	 *            timegrid units that should be sorted
	 ...
	 *
	 * @param day
	 *            day of the timegrid unit you want to find
	...
	public TimegridUnitObject findByDay(int day)
	{
		return this.stream().filter(
				timegridUnitObject -> timegridUnitObject.getDay() == day)
				.findAny().orElse(null);
	}
	 ...
	 *
	 * @param timeUnits
	 *            time units of the timegrid unit you want to find
	 * @return the time unit
	 * @since 1.0
	 */
	...
	{
		return this
				.stream().filter(timegridUnitObject -> timegridUnitObject
						.getTimeUnits().equals(timeUnits))
				.findAny().orElse(null);
	}

	/**
	 * Finds timegrid units that have the {@code day} or a part of it in their
	 * day
	 *
	 * @param day
	 *            day of the timegrid units you want to search
	 * @return {@link TimeUnits} with timegrid units that have the {@code day}
	 *         or a part of it in their day
	 * @since 1.0
	 */
	public TimegridUnits searchByDay(int day)
	{
		...
		this.stream()
				.filter(timegridUnitObject -> String
						.valueOf(timegridUnitObject.getDay())
						.contains(String.valueOf(day)))
				.forEach(timegridUnits::add);

		return timegridUnits;
	}

	/**
	 * Finds timegrid units that have the {@code timeUnits} or a part of it in
	 * their time units
	 *
	 * @param timeUnits
	 *            time units of the timegrid units you want to search
	 * @return {@link TimeUnits} with timegrid units that have the
	 *         {@code timeUnits} or a part of it in their time units
	 * @since 1.0
	 */
	public TimegridUnits searchByTimeUnits(TimeUnits timeUnits)
	{
		...
		this.forEach(timegridUnitObject -> {
			if (UntisUtils.listContainsListItem(
					timegridUnitObject.getTimeUnits(), timeUnits))
			{
	...
	public TimeUnits getTimeUnits()
	{
		...
		this.stream().map(TimegridUnitObject::getTimeUnits)
				.forEach(timeUnits::addAll);
	...
	public static class TimegridUnitObject extends ResponseObject
	{
		 ...
		 *
		 * @param day
		 *            day of the timegrid unit
		 * @param timeUnits
		 *            all time untis on the given day
		 * @since 1.0
		 */
